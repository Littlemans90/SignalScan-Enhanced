        ...
        try:
            ...
            for symbol in ticker_list:
                try:
                    ...
                    # Upfront price filter: <= $20
                    if current_price > 20.0 or current_price <= 0:
                        continue
                    ...
                    # Upfront average volume filter: >= 500k
...
class SignalScanApp(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.live_data = {k: [] for k in ["PreGap", "HOD", "RunUp", "P-HOD", "P-RunUp", "Rvsl", "Halts"]}
        self.stock_news = {}
    ...
    def categorize_stock(self, stock_data, change_pct, volume, rvol, float_shares, price, is_new_hod, is_52wk_high):
        ...
        # PreGap: <= $20, premarket, >=10% change, >=250k volume, <=100M float
        if (is_premarket and price <= 20.0 and abs(change_pct) >= 10.0 and 
            volume >= 250000 and float_shares <= 100):
        ...
        # HOD: $1-$20, new HOD, >=3.0x RVOL, <=100M float, >=+8% gain
        if (1.0 <= price <= 20.0 and is_new_hod and rvol >= 3.0 and 
            float_shares <= 100 and change_pct >= 8.0):
            ...
            if self.news_manager and ticker not in self.stock_news:
                ...
                time.sleep(0.1)
                self.news_manager.fetch_yfinance_news(ticker)
        # HOD Alt: 52-week high with >=3.0x RVOL and >=+8% gain
        elif (1.0 <= price <= 20.0 and is_52wk_high and rvol >= 3.0 and change_pct >= 8.0):
            self.live_data["HOD"].append(stock_data)
            hod_candidate_added = True
            self.enrichment_manager.record_channel_hit(ticker, "HOD")
            if self.news_manager and ticker not in self.stock_news:
        ...
        # RunUp: $1-$20, +3-15% change, >=3.0x RVOL, >=500k volume
        if (1.0 <= price <= 20.0 and 3.0 <= change_pct <= 15.0 and 
            rvol >= 3.0 and volume >= 500000):
        ...
        # P-HOD: <= $1, new HOD, >=3.0x RVOL, <=100M float, >=+8% gain
        if (price <= 1.0 and is_new_hod and rvol >= 3.0 and 
            float_shares <= 100 and change_pct >= 8.0):
            ...
            if self.news_manager and ticker not in self.stock_news:
                ...
                time.sleep(0.1)
                self.news_manager.fetch_yfinance_news(ticker)
        # P-HOD Alt: <= $1, 52-week high with >=3.0x RVOL and >=+8% gain
        elif (price <= 1.0 and is_52wk_high and rvol >= 3.0 and change_pct >= 8.0):
            self.live_data["P-HOD"].append(stock_data)
            self.enrichment_manager.record_channel_hit(ticker, "P-HOD")
            if self.news_manager and ticker not in self.stock_news:
                self.news_manager.fetch_news_pair(ticker)
        ...
        # P-RunUp: <= $1, +3-15% change, >=1.5x RVOL, >=250k volume
        if (price <= 1.0 and 3.0 <= change_pct <= 15.0 and 
            rvol >= 1.5 and volume >= 250000):
        ...
        # Rvsl: $1-$20, >=8.0x RVOL, >=8% change (keeping old logic for now)
        if (1.0 <= price <= 20.0 and rvol >= 8.0 and abs(change_pct) >= 8.0):
        ...
        # Sound alert for new HOD candidates
    ...
    def build_channel_tabs(self):
        ...
        channels = ["PreGap", "HOD", "RunUp", "P-HOD", "P-RunUp", "Rvsl", "Halts"]
